...
{
    public static Model loadModel(String file, ObjectLoader loader)
    {
        ...
        Mesh[] meshes = new Mesh[scene.mMeshes().limit()];

        for(int i = 0; i < scene.mMeshes().limit(); i++)
        {
            AIMesh mesh = AIMesh.create(scene.mMeshes().get(i));
            meshes[i] = loadMesh(mesh, loader);
        }
        Assimp.aiReleaseImport(scene);
        return new Model(meshes);
    }

    public static Model loadModel(Class<?> cls, String res, ObjectLoader loader)
    {
    ...
    private static Mesh loadMesh(AIMesh mesh, ObjectLoader loader)
    {
